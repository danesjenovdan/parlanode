  ...
  <div class="card-container card-halfling card-IME_KARTICE" :id="$options.cardData.cardData._id">
    ...
    <div class="card-content">
      <div class="card-content-front">
        ...
        <div class="tab">Seznam glasovanj</div>
...
export default {
  ...
  computed: {
    ...
    differentPeoplePlaceholder() {
      return this.selectedDifferentPeople.length > 0 ? `Izbranih: ${this.selectedDifferentPeople.length}` : 'Izberi poslance';
    },
    generatedCardUrl() {
  ...
  methods: {
    ...
    loadResults() {
      ...
      } else if ((this.selectedSamePeople.length + this.sameParties.length) === 1 && (this.selectedDifferentPeople.length + this.differentParties.length) > 0) {
        console.log('loading results');
        console.log(this.queryUrl);
        const self = this;
        $.ajax({
...
.primerjalnik {
  ...
  .primerjalnik-for, .primerjalnik-against {
    .plus {
