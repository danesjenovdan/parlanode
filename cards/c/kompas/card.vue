...
export default {
  ...
  data() {
    ...
    const partyAcronyms = this.groupBy(kompasData, (item) => [
      item.person.party.acronym,
    ]);

    const x = d3.scale.linear().range([0, width]).nice();
    ...
    const groups = [
      ...
      partyAcronyms.map((group) => ({
        label: group[0].person.party.acronym,
        items: group.map((p) => p.person.id),
      }))
    );

    const { parlaState } = this.$options.cardData;
    const state =
      parlaState.people && parlaState.parties
        ? parlaState
        : { people: [], parties: [] };
  ...
  computed: {
    peoplePlaceholder() {
      const numberOfSelectedPeople = this.people.filter((p) => p.selected)
        .length;
    ...
    url() {
      return `${this.slugs.urls.glej}/${
        import.meta.env.CARD_NAME
  ...
  methods: {
    ...
    zoom(animate) {
      ...
      if (animate) {
        svg
          ...
          .attr(
            'transform',
            (d) => `translate(${this.x(d.score.vT1)},${this.y(d.score.vT2)})`
          );
      } else {
        svg
          .selectAll('.dot')
    ...
    showPersonPicture(datum) {
      ...
      d3.select(`#kompas-personcard${datum.person.id}`).classed(
        'hidden',
        false
      );
    ...
    hidePersonPicture(datum) {
      // hide person image
      d3.select(`#kompas-personcard${datum.person.id}`).classed('hidden', true);
    ...
    showPartyPictures(acronym) {
      ...
      people.forEach((p) => {
        this.showPersonPicture(
          this.kompasData.find((pp) => pp.person.id === p.id)
        );
      });
    ...
    zoomIn(datum, scale) {
      ...
      const newx =
        (parentBounds.width / 2) * stretchCoef.x - this.x(datum.score.vT1);
      const newy =
        (parentBounds.height / 2) * stretchCoef.y - this.y(datum.score.vT2);
      ...
      if (parseInt(id, 10)) {
        ...
        if (person) {
          if (!person.selected) {
            ...
            this.showPersonPicture(
              this.kompasData.find((p) => p.person.id === id)
            );
          } else {
    ...
    drawCompass() {
      ...
      svg.append('g').classed('y axis', true).call(this.yAxis);

      svg.selectAll('.tick').each(function _() {
        this.remove();
      });
      ...
      objects
        ...
        .attr(
          'id',
          (d) =>
            `kompasgroup${d[0].person.party.acronym.replace(/[ +,]/g, '_')}`
        );
      ...
      this.partyAcronyms.forEach((group) => {
        svg
          .select(
            `#kompasgroup${group[0].person.party.acronym.replace(
              /[ +,]/g,
              '_'
            )}`
          )
          ...
          .on('mouseover', (d) => {
            ...
            tooltipdiv
              ...
              .style(
                ...
                `${
                  d3.event.pageX -
                  tooltipdiv.node().getBoundingClientRect().width / 2 -
