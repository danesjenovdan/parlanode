<template>
  ...
  >
    <div class="result-container">
      ...
      <div class="law-title">{{ $options.contextData.cardData.text }}</div>
      <result
    ...
    >
      <p-tab
        v-if="data.abstract /* SL override TODO: fix */ || true"
        :label="$t('abstract')"
        variant="dark"
      >
        <excerpt
...
export default {
  ...
  data() {
    const documents = this.$options.contextData.cardData.votes.reduce(
      (prev, cur) => {
        if (cur.documents) {
    ...
    const title = this.$options.contextData.cardState.fullName
      ? `${this.$options.contextData.cardData.text.slice(0, 100)}...`
      : 'Zakon';

    // TODO: i18n
    // did we have "glasovanje o zakonu v celoti"
    const finalVoteExists =
      this.$options.contextData.cardData.votes.filter(
    ...
    if (finalVoteExists) {
      // TODO: i18n
      const filteredVote = this.$options.contextData.cardData.votes.filter(
        (vote) => vote.text.indexOf('v celoti') > -1
      )[0];
      const vote = {
    ...
    let startTab = 0;
    if (this.$options.contextData.cardState.selectedTab) {
      startTab = this.$options.contextData.cardState.selectedTab;
    }
