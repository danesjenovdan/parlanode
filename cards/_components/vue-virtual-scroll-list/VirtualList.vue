<script>
/*
 * This is "vue-virtual-scroll-list" from https://github.com/tangbc/vue-virtual-scroll-list
 * Modified to use the global Vue instance.
 */
/* eslint-disable indent, object-shorthand, func-names, no-var, semi, one-var,
   vue/require-default-prop, comma-dangle, vue/order-in-components, vars-on-top,
   no-else-return, no-plusplus, max-len, one-var-declaration-per-line, no-bitwise,
...
export default {
  ...
  methods: {
    ...
    filter: function () {
      ...
      if (this.variable) {
        ...
        paddingBottom =
          this.size * (hasPadding ? delta.total - delta.keeps : 0) - paddingTop;
      }
  ...
  mounted: function () {
    if (this.start) {
      var { start } = this.getZone(this.start);
      this.setScrollTop(
        this.variable ? this.getVarOffset(start) : start * this.size
      );
    } else if (this.offset) {
  ...
  render: function (h) {
    ...
    return h(
      this.rtag,
      {
        ...
        on: {
          '&scroll': dbc
            ? _debounce(this.onScroll.bind(this), dbc)
            : this.onScroll,
        },
      },
      [
